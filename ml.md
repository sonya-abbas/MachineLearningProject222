...
# Executive Summary
The objective of this project is to measure the quality of performing a set of activities done by a group of people. In order to do so, we need to build a predictive model that allow us to predict the class of performance based on a set of indicators/predictors. This report will rake us through all the steps we followes to get a very accurate model that score 100% accuracy when it has been applied on test data (20 cases). more specifically, for each observation, we need to predict if it is (Class A -exactly according to the specification-, Class B -throwing the elbows to the front-, Class C -lifting the dumbbell only halfway-, Class D -lowering the dumbbell only halfway-, Class E -throwing the hips to the front-)
...
As a first of pre-processing, we do some exploratory data analysis to understand the data we have. We apply different functions such as dim, names, str, summary..etc
...
names(ds_train)
```
The data summery shows a number of columns where there is a big number of NA's and others with a big number of empty values. so we dig more 
as Figure 1 below shows the distrubution of NA's values per columns. We notice a very high number of NA's for some olumns and we see a clear pattern of NA's values per rows. It is equal (either NA's or empty values) to 15386 from 15699 in trainig dataset portion and 19216/19622 in the original training dataset. 
...
We define a threshold for calculating correlations betweeen predictors. Here is a list of predictors names which are coorelated:

```r
descrCor <-  cor(ds_train_dumm[, -56])
highlyCorDescr <- findCorrelation(descrCor, cutoff = .75)
ds_train_noCor <- ds_train_dumm[,-highlyCorDescr]
#descrCor2 <- cor(ds_train_noCor[, -35])
...
## [22] "new_window.yes"
```
Figure2 below shows a set of correlated variables.
...
```
## Warning in qr.default(object): NAs introduced by coercion
```
...
```
## Error in preProcess.default(ds_train_noCor[, -35], method = c("center", : all columns of x must be numeric
```

```
## Error in predict(preProcValues, ds_train_noCor[, -35]): object 'preProcValues' not found
```
...
##  "Min.   :-48.00  " "Min.   :354.0  " "Min.   :-180.00  "
##    pitch_arm            yaw_arm            total_accel_arm  
##  "Min.   :-88.800  " "Min.   :-180.0000  " "Min.   : 1.00  "
##   gyros_arm_y         gyros_arm_z         magnet_arm_x     magnet_arm_z   
##  "Min.   :-3.4400  " "Min.   :-2.3300  " "Min.   :-584  " "Min.   :-597  "
##  roll_dumbbell       pitch_dumbbell       yaw_dumbbell       
##  "Min.   :-153.71  " "Min.   :-149.59  " "Min.   :-150.871  "
##  total_accel_dumbbell gyros_dumbbell_y     magnet_dumbbell_z  
##  "Min.   : 0.0  "     "Min.   :-2.10000  " "Min.   :-262.00  "
##   roll_forearm        pitch_forearm       yaw_forearm       
##  "Min.   :-180.000  " "Min.   :-72.50  " "Min.   :-180.00  "
##  total_accel_forearm gyros_forearm_x      gyros_forearm_y      
##  "Min.   :  0.00  "  "Min.   :-22.0000  " "Min.   : -7.02000  "
##  accel_forearm_x     accel_forearm_z     magnet_forearm_x   
##  "Min.   :-498.00  " "Min.   :-446.00  " "Min.   :-1280.0  "
##  magnet_forearm_y   magnet_forearm_z   classe    
##  "Min.   :-896.0  " "Min.   :-973.0  " "A:4464  "
```

```
## Error in summary(trainTransformed): object 'trainTransformed' not found
```
...
```
## Error in eval(expr, envir, enclos): object 'trainTransformed' not found
```
...
## 1- Feature Selection $ Cross Validation
...
```
## Error in predict(model, ds_test): object 'model' not found
```
